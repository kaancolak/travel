...
template <typename PointType> bool point_z_cmp(PointType a, PointType b) {
  return a.z < b.z;
}
...
template <typename PointType> struct TriGridNode {
  int node_type;
...
public:
  ...
  void setParams(const double max_range, const double min_range,
                 const double resolution, const int num_iter, const int num_lpr,
                 const int num_min_pts, const double th_seeds,
                 const double th_dist, const double th_outlier,
                 const double th_normal, const double th_weight,
                 const double th_lcc_normal_similiarity,
                 const double th_lcc_planar_model_dist,
                 const double th_obstacle, const bool refine_mode,
...
private:
  ...
  bool filterPoint(const PointType &pt_in) {
    ...
    if (xy_range >= MAX_RANGE_ || xy_range <= MIN_RANGE_)
      return true;
  ...
  void embedCloudToTriGridField(const pcl::PointCloud<PointType> &cloud_in,
    ...
    for (auto const &pt : cloud_in.points) {
      ...
      if (angle >= (M_PI / 4) && angle < (3 * M_PI / 4)) {
        ...
        if (!tgf_out[r_i][c_i][1].is_curr_data) {
          tgf_out[r_i][c_i][1].is_curr_data = true;
        }
      } else if (angle >= (-M_PI / 4) && angle < (M_PI / 4)) {
        // upper side
        tgf_out[r_i][c_i][0].ptCloud.push_back(pt);
        if (!tgf_out[r_i][c_i][0].is_curr_data) {
          tgf_out[r_i][c_i][0].is_curr_data = true;
        }

      } else if (angle >= (-3 * M_PI / 4) && angle < (-M_PI / 4)) {
        // right side
        tgf_out[r_i][c_i][3].ptCloud.push_back(pt);
        if (!tgf_out[r_i][c_i][3].is_curr_data) {
          tgf_out[r_i][c_i][3].is_curr_data = true;
        }
      } else {
        // lower side
        tgf_out[r_i][c_i][2].ptCloud.push_back(pt);
        if (!tgf_out[r_i][c_i][2].is_curr_data) {
          tgf_out[r_i][c_i][2].is_curr_data = true;
        }
      }
    }
  ...
  void extractInitialSeeds(const pcl::PointCloud<PointType> &p_sorted,
    ...
    for (int i = 0; i < (int)p_sorted.points.size(); i++) {
      if (p_sorted.points[i].z < lpr_height + TH_SEEDS_) {
        if (p_sorted.points[i].z < lpr_height - TH_OUTLIER_)
          continue;
  ...
  void estimatePlanarModel(const pcl::PointCloud<PointType> &ground_in,
                           TriGridNode<PointType> &node_out) {
  ...
  void modelPCAbasedTerrain(TriGridNode<PointType> &node_in) {
    ...
    sort(sort_ptCloud.points.begin(), sort_ptCloud.end(),
         point_z_cmp<PointType>);
  ...
  double calcNodeWeight(const TriGridNode<PointType> &node_in) {
    ...
    // weight = (node_in.singular_values[0]/node_in.singular_values[2] +
    // node_in.singular_values[1]/node_in.singular_values[2])/(node_in.singular_values[0]/node_in.singular_values[1]);
  ...
  void findDominantNode(const TriGridField<PointType> &tgf_in,
  ...
  void searchAdjacentNodes(const TriGridIdx &cur_idx,
                           std::vector<TriGridIdx> &adjacent_idxs) {
    adjacent_idxs.clear();
  ...
  bool LocalConvecityConcavity(const TriGridField<PointType> &tgf,
    ...
    if ((abs(dist_s2t) > TH_DIST_to_planar ||
  ...
  void BreadthFirstTraversableGraphSearch(TriGridField<PointType> &tgf_in) {
    ...
    while (!searching_idx_queue.empty()) {
      ...
      for (int i = 0; i < (int)neighbor_idxs.size(); i++) {
        ...
        if (max_planar_height < tgf_in[n_i.row][n_i.col][n_i.tri].mean_pt[2])
          max_planar_height = tgf_in[n_i.row][n_i.col][n_i.tri].mean_pt[2];

        tgf_in[n_i.row][n_i.col][n_i.tri].node_type = GROUND;
        tgf_in[n_i.row][n_i.col][n_i.tri].is_rejection = false;
        tgf_in[n_i.row][n_i.col][n_i.tri].depth =
            tgf_in[current_node_idx.row][current_node_idx.col]
        ...
        for (int r_i = 0; r_i < rows_; r_i++) {
          for (int c_i = 0; c_i < cols_; c_i++) {
            for (int s_i = 0; s_i < (int)tgf_in[r_i][c_i].size(); s_i++) {
              if (tgf_in[r_i][c_i][s_i].is_visited) {
                continue;
              }
              ...
              // if (tgf_in[r_i][c_i][s_i].mean_pt[2] >= max_planar_height+1) {
              // continue; }

              if (tgf_in[r_i][c_i][s_i].depth >= 0) {
                continue;
              }
  ...
  double getCornerWeight(const TriGridNode<PointType> &node_in,
  ...
  void setTGFCornersCenters(
      const TriGridField<PointType> &tgf_in,
      ...
      for (int c_i = 0; c_i < cols_; c_i++) {
        ...
        for (int s_i = 0; s_i < (int)tgf_in[r_i][c_i].size(); s_i++) {
          ...
          case 0: // upper Tri-grid bin
            // RT / LT / C
            trigrid_corners_out[r_i + 1][c_i].zs.push_back(
                (-tgf_in[r_i][c_i][s_i].normal(0, 0) * corner_TR.x -
                 tgf_in[r_i][c_i][s_i].normal(1, 0) * corner_TR.y -
                 tgf_in[r_i][c_i][s_i].d) /
                tgf_in[r_i][c_i][s_i].normal(2, 0));
            trigrid_corners_out[r_i + 1][c_i].weights.push_back(
                getCornerWeight(tgf_in[r_i][c_i][s_i], corner_TR));

            trigrid_corners_out[r_i + 1][c_i + 1].zs.push_back(
                (-tgf_in[r_i][c_i][s_i].normal(0, 0) * corner_TL.x -
                 tgf_in[r_i][c_i][s_i].normal(1, 0) * corner_TL.y -
                 tgf_in[r_i][c_i][s_i].d) /
                tgf_in[r_i][c_i][s_i].normal(2, 0));
            trigrid_corners_out[r_i + 1][c_i + 1].weights.push_back(
                getCornerWeight(tgf_in[r_i][c_i][s_i], corner_TL));

            trigrid_centers_out[r_i][c_i].zs.push_back(
                (-tgf_in[r_i][c_i][s_i].normal(0, 0) * corner_C.x -
                 tgf_in[r_i][c_i][s_i].normal(1, 0) * corner_C.y -
                 tgf_in[r_i][c_i][s_i].d) /
                tgf_in[r_i][c_i][s_i].normal(2, 0));
            trigrid_centers_out[r_i][c_i].weights.push_back(
                getCornerWeight(tgf_in[r_i][c_i][s_i], corner_C));
          ...
          case 1: // left Tri-grid bin
            ...
            trigrid_corners_out[r_i][c_i + 1].zs.push_back(
                (-tgf_in[r_i][c_i][s_i].normal(0, 0) * corner_BL.x -
                 tgf_in[r_i][c_i][s_i].normal(1, 0) * corner_BL.y -
                 tgf_in[r_i][c_i][s_i].d) /
                tgf_in[r_i][c_i][s_i].normal(2, 0));
            trigrid_corners_out[r_i][c_i + 1].weights.push_back(
                getCornerWeight(tgf_in[r_i][c_i][s_i], corner_BL));
          ...
          case 2: // lower Tri-grid bin
            ...
            trigrid_corners_out[r_i][c_i].zs.push_back(
                (-tgf_in[r_i][c_i][s_i].normal(0, 0) * corner_BR.x -
                 tgf_in[r_i][c_i][s_i].normal(1, 0) * corner_BR.y -
                 tgf_in[r_i][c_i][s_i].d) /
                tgf_in[r_i][c_i][s_i].normal(2, 0));
            trigrid_corners_out[r_i][c_i].weights.push_back(
                getCornerWeight(tgf_in[r_i][c_i][s_i], corner_BR));
  ...
  void updateTGFCornersCenters(
    ...
    for (int r_i = 0; r_i < rows_ + 1; r_i++) {
      for (int c_i = 0; c_i < cols_ + 1; c_i++) {
        if (trigrid_corners_out[r_i][c_i].zs.size() > 0 &&
            trigrid_corners_out[r_i][c_i].weights.size() > 0) {
    ...
    for (int r_i = 0; r_i < rows_; r_i++) {
      for (int c_i = 0; c_i < cols_; c_i++) {
        if (trigrid_centers_out[r_i][c_i].zs.size() > 0 &&
            trigrid_centers_out[r_i][c_i].weights.size() > 0) {
          updated_center = getMeanCorner(trigrid_centers_out[r_i][c_i]);
          trigrid_centers_out[r_i][c_i] = updated_center;
          // trigrid_centers_out[r_i][c_i].z =
          // get_mean(trigrid_centers_out[r_i][c_i].zs,trigrid_centers_out[r_i][c_i].weights);
        } else {
    ...
    for (int r_i = 0; r_i < rows_; r_i++) {
      for (int c_i = 0; c_i < cols_; c_i++) {
        for (int s_i = 0; s_i < (int)tgf_out[r_i][c_i].size(); s_i++) {
          ...
          switch (s_i) {
          case 0:
            if (trigrid_corners_in[r_i + 1][c_i].zs.size() == 0 ||
                trigrid_corners_in[r_i + 1][c_i + 1].zs.size() == 0 ||
                trigrid_centers_in[r_i][c_i].zs.size() == 0) {
            ...
            break;
          case 1:
            if (trigrid_corners_in[r_i + 1][c_i + 1].zs.size() == 0 ||
                trigrid_corners_in[r_i][c_i + 1].zs.size() == 0 ||
                trigrid_centers_in[r_i][c_i].zs.size() == 0) {
              if (tgf_out[r_i][c_i][s_i].node_type != NONGROUND) {
                tgf_out[r_i][c_i][s_i].node_type = UNKNOWN;
              }
            ...
            break;
          case 2:
            if (trigrid_corners_in[r_i][c_i + 1].zs.size() == 0 ||
                trigrid_corners_in[r_i][c_i].zs.size() == 0 ||
                trigrid_centers_in[r_i][c_i].zs.size() == 0) {
              if (tgf_out[r_i][c_i][s_i].node_type != NONGROUND) {
                tgf_out[r_i][c_i][s_i].node_type = UNKNOWN;
              }
            ...
            refined_corner_1 =
                convertCornerToEigen(trigrid_corners_in[r_i][c_i + 1]);
            refined_corner_2 =
                convertCornerToEigen(trigrid_corners_in[r_i][c_i]);
            refined_center = convertCornerToEigen(trigrid_centers_in[r_i][c_i]);
            break;
          case 3:
            if (trigrid_corners_in[r_i][c_i].zs.size() == 0 ||
                trigrid_corners_in[r_i + 1][c_i].zs.size() == 0 ||
                trigrid_centers_in[r_i][c_i].zs.size() == 0) {
              if (tgf_out[r_i][c_i][s_i].node_type != NONGROUND) {
                tgf_out[r_i][c_i][s_i].node_type = UNKNOWN;
              }
          ...
          //     std::cout << "Origin normal: " << tgf_out[r_i][c_i][s_i].normal
          //     << std::endl; std::cout << "Update normal: " << udpated_normal
          //     << std::endl;
          // }
          if (udpated_normal(2, 0) < TH_NORMAL_) { // non-planar
            ...
            updated_mean_pt[0] = (refined_corner_1[0] + refined_corner_2[0] +
                                  refined_center[0]) /
                                 3;
            updated_mean_pt[1] = (refined_corner_1[1] + refined_corner_2[1] +
                                  refined_center[1]) /
                                 3;
            updated_mean_pt[2] = (refined_corner_1[2] + refined_corner_2[2] +
                                  refined_center[2]) /
                                 3;
  ...
  void
  segmentTGFGround_developing(const TriGridField<PointType> &tgf_in,
                              pcl::PointCloud<PointType> &ground_cloud_out,
                              pcl::PointCloud<PointType> &nonground_cloud_out,
                              pcl::PointCloud<PointType> &obstacle_cloud_out,
                              pcl::PointCloud<PointType> &outlier_cloud_out) {
          ...
          if (tgf_in[r_i][c_i][s_i].node_type == GROUND) {
            ...
            if (adj_idx_vec.empty()) {
              ...
              for (int adj_i = 0; adj_i < adj_idx_vec.size(); adj_i++) {
                ...
                is_adjacent = true;
                if (getTriGridNode(adj_idx_vec[adj_i]).weight >
                    highest_weight) {
              ...
              if (is_adjacent) {
                TriGridNode<PointType> tmp_node =
                    getTriGridNode(highest_adj_tri_idx);
                tmp_node.ptCloud = getTriGridNode(curr_tgf_idx).ptCloud;
                segmentNodeGround(tmp_node, ptCloud_nodewise_ground_,
                                  ptCloud_nodewise_nonground_,
                                  ptCloud_nodewise_obstacle_,
                                  ptCloud_nodewise_outliers_);
              } else {
  ...
  setPlanarModel(const TriGridNode<PointType> &node_in,
                 const TriGridIdx &node_idx) {
    geometry_msgs::msg::PolygonStamped polygon_out;
    polygon_out.header = msg_header_;
    geometry_msgs::msg::Point32 corner_0, corner_1, corner_2;
    ...
    if (node_in.node_type == GROUND) {
      ...
      case 0:
        // topx lowy & topx topy
        corner_1.x = (r_i + 1) * TGF_RESOLUTION_ + tgf_min_x;
        corner_1.y = (c_i)*TGF_RESOLUTION_ + tgf_min_y;
        corner_1.z = (-node_in.normal(0, 0) * corner_1.x -
                      node_in.normal(1, 0) * corner_1.y - node_in.d) /
                     node_in.normal(2, 0);

        corner_2.x = (r_i + 1) * TGF_RESOLUTION_ + tgf_min_x;
        corner_2.y = (c_i + 1) * TGF_RESOLUTION_ + tgf_min_y;
        corner_2.z = (-node_in.normal(0, 0) * corner_2.x -
                      node_in.normal(1, 0) * corner_2.y - node_in.d) /
                     node_in.normal(2, 0);

        corner_0.x = (r_i + 0.5) * TGF_RESOLUTION_ + tgf_min_x;
        corner_0.y = (c_i + 0.5) * TGF_RESOLUTION_ + tgf_min_y;
        corner_0.z = (-node_in.normal(0, 0) * corner_0.x -
                      node_in.normal(1, 0) * corner_0.y - node_in.d) /
                     node_in.normal(2, 0);
        break;
      case 1:
        // topx topy & lowx topy
        corner_1.x = (r_i + 1) * TGF_RESOLUTION_ + tgf_min_x;
        corner_1.y = (c_i + 1) * TGF_RESOLUTION_ + tgf_min_y;
        corner_1.z = (-node_in.normal(0, 0) * corner_1.x -
                      node_in.normal(1, 0) * corner_1.y - node_in.d) /
                     node_in.normal(2, 0);

        corner_2.x = (r_i)*TGF_RESOLUTION_ + tgf_min_x;
        corner_2.y = (c_i + 1) * TGF_RESOLUTION_ + tgf_min_y;
        corner_2.z = (-node_in.normal(0, 0) * corner_2.x -
                      node_in.normal(1, 0) * corner_2.y - node_in.d) /
                     node_in.normal(2, 0);
      ...
      case 2:
        // lowx topy & lowx lowy
        corner_1.x = (r_i)*TGF_RESOLUTION_ + tgf_min_x;
        corner_1.y = (c_i + 1) * TGF_RESOLUTION_ + tgf_min_y;
        corner_1.z = (-node_in.normal(0, 0) * corner_1.x -
                      node_in.normal(1, 0) * corner_1.y - node_in.d) /
                     node_in.normal(2, 0);
        ...
        corner_2.x = (r_i)*TGF_RESOLUTION_ + tgf_min_x;
        corner_2.y = c_i * TGF_RESOLUTION_ + tgf_min_y;
        corner_2.z = (-node_in.normal(0, 0) * corner_2.x -
                      node_in.normal(1, 0) * corner_2.y - node_in.d) /
                     node_in.normal(2, 0);
